    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto text-right hidden sm:block">
          ...
          <p className={`text-xl font-black ${isCancelled ? 'text-red-600' : 'text-[#2E7D32]'}`}>{order.orderStatus.toUpperCase()}</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {/* Tracking Timeline */}
        <div className="bg-white rounded-2xl p-6 md:p-8 border border-stone-200 shadow-sm relative overflow-hidden">
              ...
              {TRACKING_STEPS.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-1">
                      <p className={`text-sm font-bold ${isCurrent ? (isCancelled ? 'text-red-700' : 'text-stone-900') : isCompleted ? 'text-stone-700' : 'text-stone-400'}`}>
                        {step.label}
                      </p>
                      <div className="flex flex-col mt-0.5">
                        <p className="text-[11px] font-semibold text-stone-500">{step.date}</p>
                        {isCompleted && step.location && (
                          <p className={`text-[11px] font-medium mt-1 flex items-center gap-1 ${isCancelled ? 'text-red-400' : 'text-stone-400'}`}>
                    ...
                    {isCurrent && (
                      <span className={`absolute right-0 top-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md ${isCancelled ? 'text-red-600 bg-red-50' : 'text-[#2E7D32] bg-[#F0FAF0]'}`}>
                        Current
                      </span>
                    )}
                  </motion.div>
