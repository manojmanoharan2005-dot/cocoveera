...
export const ProductGrid = ({ loading, children }) => {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-8">
        {[...Array(8)].map((_, idx) => (
          <div 
            key={idx} 
  ...
  return (
    <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-4 md:gap-5">
      {children}
    </div>
  );
};
